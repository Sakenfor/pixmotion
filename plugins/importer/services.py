    ...
    def import_from_url(self, url: str) -> str | None:
        ...
        try:
            ...
            if not img_tag or not img_tag.get('content'):
                self.log.error("Could not find high-resolution image meta tag on Pinterest page.")
                return None
            ...
            self.log.info(f"Successfully downloaded and saved Pinterest image to: {filepath}")
            return filepath
            ...
            self.log.error(f"An unexpected error occurred during Pinterest import: {e}", exc_info=True)
            return None